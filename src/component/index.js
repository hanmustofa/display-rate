...
function Change() {
  ...
  function doFetch() {
    fetch(
      "http://api.exchangeratesapi.io/v1/latest?access_key=83a6e2f0ee2056936f61c8c329ee4dad&format=1" +
        selectedBase
    )
        ...
        <div className="Graph">
          <div className="Graph-select">
            <div>
              ...
              <select
                onChange={(event) => setSelectedRegionTitle(event.target.value)}
              >
          ...
          <div className="Graph-text">
            {rates
              .sort()
              .filter((rate) =>
                selectedRegion[selectedRegionTitle].includes(rate[0])
              )
              .map((rate) => (
